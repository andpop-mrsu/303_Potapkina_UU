    ...
    <form method="post" action="help_add_worker.php" enctype="application/x-www-form-urlencoded">
        ...
        <fieldset>
            ...
            <?php foreach ($event->getSpecialization() as $item): ?>
                <p><label> <input type="radio" name="specialization" value="<?= $item['id'] ?>"> <?= $item['title'] ?>
                    </label></p>
            <?php endforeach; ?>
        </fieldset>
        ...
        <fieldset>
            ...
            <p><label> <input type="radio" name="status" value="не является работником"> Не является работником </label>
            </p>
        </fieldset>
